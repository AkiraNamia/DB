...
--Триггер – это особый вид хранимой процедуры, предназначенной для обработки событий в БД. Поддерживается два типа триггеров: DDL-триггеры и DML-триггеры. 
--DML-триггеры бывают двух типов: AFTER-триггеры и INSTEAD OF-триггеры. 
--Триггеры типа AFTER исполняются после выполнения оператора, вызвавшего соответствующее событие. При этом создаются автоматически две псевдотаб-лицы INSERTED и DELETED. 
...
go --записывает данные в таблицу
...
insert into TEACHER
values ('БЖК', 'Божко Денис Владимирович', 'м', 'ИСиТ')
update TEACHER set TEACHER = 'БЖКЖК' where TEACHER = 'БЖК' 
delete from TEACHER where TEACHER = 'БЖК'
...
insert into TEACHER values ('БЖК', 'Божко Денис Владимирович', 'м', 'ИСиТ')
delete from TEACHER where TEACHER = 'БЖК'
...
use БожкоЛаба2Курсы
go
create trigger TR_GROUPS_DELETE on Группы instead of DELETE
as
raiserror('Удаление запрещено', 10,1)
go
select * from Группы
delete from Группы where [Номер группы] = 1

--drop trigger TR_GROUPS_DELETE
