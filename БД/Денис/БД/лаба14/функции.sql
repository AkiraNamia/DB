use ФАЙЛОВАЯ_ГРУППА --Функция – это объект БД, представляющий собой поименованный код T-SQL.

--Отличие функций от хранимых процедур в ограничениях, накладываемых на код функции, в форме представления ре-зультата работы,
--а также в способе вызова. В функции не допускается применение DDL-операторов create alter drop, DML-операторов, 
--изменяющих БД (INSERT, DELETE, UPDATE), конструкций TRY/CATCH, а также использование транзакций. 
--не возвращает time, image, text,


--скалярная - возвращает одно значение
--табличная - возвращает таблицу
--встроенные
--многооператорные

go
create function CountStudents(@faculty varchar(20)) returns int --1. функция подсчета студентов по факультету
as
begin
	declare @count int = 0
...
go
create function PulpitSubjects(@pulpit varchar(20)) returns varchar(300) --2. скалярная функция + отчет
as
begin
...
go --4. код кафедры => количество преподов
...
use БожкоЛаба2Курсы; --5. для своей бд
go
create function GroupInfo(@number int) returns int 
as
begin
	declare @count int = 0
	set @count = (select [Номер группы] from Группы where Группы.[Количество студентов] = @number)
	return @count
end
go

select dbo.GroupInfo(23)
